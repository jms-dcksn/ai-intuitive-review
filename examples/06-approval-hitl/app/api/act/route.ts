...
import { feedSnapshot, getOrCreateThread, getThread, ThreadRecorder } from "@/lib/thread";
import type { ApprovalUIMessage, Resolution } from "@/lib/types";

export const runtime = "nodejs";
// A live slice can run several model calls before the next approval.
...
export async function POST(req: Request): Promise<Response> {
  ...
  const { thread, created } = getOrCreateThread(sessionId, mocked);

  const stream = createUIMessageStream<ApprovalUIMessage>({
    execute: async ({ writer }) => {
      ...
      if (body.resolution && created) {
        ...
        // wiped. Replaying from scratch would re-propose actions the client
        // already resolved and dead-end the UI — say so instead.
        rec.write({
          type: "data-done",
          data: {
            summary:
              "The server lost this run's state (dev reload mid-run). Hit Restart to run again.",
            stats: "session lost",
          },
        });
        return;
...
/**
 * Rehydrate a run after a page refresh. The feed is server-authoritative (held
 * in the thread), so the client can rebuild its whole state from this instead
 * of losing everything the way a client-only feed would.
 */
export async function GET(req: Request): Promise<Response> {
  const sessionId = new URL(req.url).searchParams.get("sessionId");
  const thread = sessionId ? getThread(sessionId) : undefined;
  if (!thread) return Response.json({ found: false });
  return Response.json({ found: true, ...feedSnapshot(thread) });
}
